...
class CSOptimization:
    def __init__(self, num_users, num_cells, env, colony_size=30, iterations=50, pa=0.25):
        ...
        self.pa = pa
        self.population = [np.random.randint(0, num_cells, size=num_users) for _ in range(colony_size)]
    
    def fitness(self, solution):
        return evaluate_solution(self.env, solution)["fitness"]
    
    def levy_flight(self):
        return np.random.randint(-1, 2, size=self.env.num_users)
    ...
    def optimize(self):
        for _ in range(self.iterations):
            ...
            for idx in indices[:num_abandon]:
                self.population[idx] = np.random.randint(0, self.env.num_cells, size=self.env.num_users)
