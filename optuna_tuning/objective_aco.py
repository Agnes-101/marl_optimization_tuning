# File: optuna_tuning/objective_aco.py
import sys
import os
import optuna
import time
...
def train_aco(trial, fixed_env_args):
    # Hyperparameters to tune
    params = {
        'iterations': int(trial.suggest_int('iterations', 50, 300)),  # Match PFO range
        'population_size': int(trial.suggest_int('population_size', 30, 100)),
        'aco_alpha': float(trial.suggest_float('aco_alpha', 0.5, 2.0)),
        'aco_beta': float(trial.suggest_float('aco_beta', 0.5, 2.0)),
        'evaporation_rate': float(trial.suggest_float('evaporation_rate', 0.05, 0.2)),
    ...
    aco = ACOOptimization(
        ...
        env=env.env,
        ants=params['population_size'],
        iterations=params['iterations'],
        evaporation_rate=params['evaporation_rate'],
        alpha=params['aco_alpha'],
        beta=params['aco_beta'],
        seed=42  # Add if your ACO implementation supports seeding
    )
...
def objective(trial):
    # Fixed environment configuration (identical to PFO/GWO)
    env_args = {
        "num_users": 60,
        "episode_limit": 50,
    ...
    return train_aco(trial, env_args)
